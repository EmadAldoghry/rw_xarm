...
def launch_setup(context, *args, **kwargs):
    # =================================================================
    # === 1. GATHER LAUNCH ARGUMENTS & CORRECT TYPES
    # =================================================================
    ...
    # <<< FIX: Correctly handle parameter types >>>
    dof_arg = LaunchConfiguration('dof')
    dof_str = dof_arg.perform(context)  # Use string for xacro
    dof_int = int(dof_str)              # Use integer for C++ nodes
    ...
    moveit_config = (
        MoveItConfigsBuilder(
            context=context, dof=dof_str, robot_type=robot_type, prefix=prefix, hw_ns=hw_ns, limited=limited,
    ...
    # <<< FIX: Remap robot_state_publisher to the merged topic >>>
    ...
    # <<< FIX: Configure joint_state_publisher to merge the correct sources >>>
    joint_state_publisher_node = Node(
        ...
        parameters=[
            {'use_sim_time': use_sim_time,
             'source_list': ['/mobile_base/joint_states', '/joint_state_broadcaster/joint_states']}
        ],
        remappings=[('/joint_states', '/merged_joint_states')]
    )
    ...
    # <<< FIX: Remap move_group to the merged topic >>>
    ...
    # <<< FIX: Pass correct parameters to the planner node and remap it >>>
    ...
    xarm_type_str = '{}{}'.format(robot_type.perform(context), dof_str)
