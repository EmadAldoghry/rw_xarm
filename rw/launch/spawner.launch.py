...
def launch_setup(context, *args, **kwargs):
    ...
    # --- Joint State Publisher for non-ros2_control joints (wheels, etc.) ---
    # <<< FIX #2: Use non-GUI publisher and specify the source list >>>
    # This tells the publisher to listen to the arm states from ros2_control and only publish the missing ones.
    joint_state_publisher_node = Node(
        ...
        parameters=[
            {'use_sim_time': use_sim_time,
             'source_list': ['/joint_states_throttle']} # Listen to the ros2_control topic
        ],
        remappings=[('/joint_states', '/joint_states_gui_only')] # Remap its output to avoid conflict
    )
    
    # --- Throttle node for ros2_control joint states ---
    # The joint_state_publisher needs a source topic. We throttle the main topic for it.
    joint_states_throttle_node = Node(
        package='topic_tools',
        executable='throttle',
        name='joint_states_throttle',
        arguments=['messages', '/joint_states', '10', '/joint_states_throttle'],
    ...
    arm_controller_spawner = Node(
        ...
        arguments=[f'{xarm_type}_traj_controller', '-c', '/controller_manager'],
    )
...
def generate_launch_description():
    declared_arguments = [
        ...
        # <<< FIX #1: Correct the default value for DOF
